...
--
--
UPDATE alf_prop_string_value as apsv1
INNER JOIN (
    SELECT id
...
--
UPDATE alf_prop_value AS apv
INNER JOIN alf_prop_string_value_duplicated_temp AS apsvdt ON (
    apv.long_value = apsvdt.after_problem_sv_id
)
SET
...
WHERE
    apv.actual_type_id IN (
        SELECT MAX(id)
        FROM alf_prop_class AS apc
        WHERE
            apc.java_class_name = 'java.lang.String'
    )
...
--
UPDATE alf_prop_value AS apv
INNER JOIN alf_prop_value_not_duplicated_temp AS apvndt
SET
...
--
UPDATE alf_prop_unique_ctx AS apuc
INNER JOIN alf_prop_unique_ctx_map_values_temp AS apucmvt ON (
    apucmvt.prop1_id_after_problem = apuc.prop1_id
    AND apuc.value1_prop_id = apucmvt.value1_after_problem
...
INNER JOIN alf_prop_unique_ctx_map_values_temp AS apucmvt ON (
    apucmvt.prop1_id_after_problem = apuc.prop1_id
    AND apuc.value2_prop_id = apucmvt.value2_after_problem
    AND apucmvt.value2_before_problem IS NOT NULL
)
SET
...
INNER JOIN alf_prop_unique_ctx_map_values_temp AS apucmvt ON (
    apucmvt.prop1_id_after_problem = apuc.prop1_id
    AND apuc.value3_prop_id = apucmvt.value3_after_problem
    AND apucmvt.value3_before_problem IS NOT NULL
)
SET
...
--
UPDATE alf_prop_link AS apl
INNER JOIN alf_prop_value_duplicated_temp AS apvdt
SET
...
--
UPDATE alf_audit_entry AS aae
INNER JOIN alf_audit_app_duplicated_temp AS aaadt
SET
